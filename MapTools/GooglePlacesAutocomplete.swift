...
open class Place: NSObject {
  ...
  /**
    Call Google Place Details API to get detai
...
class GooglePlacesRequestHelpers {
  ...
  */
  ...
  fileprivate class func doRequest(_ url: String, params: [String: String], success: @escaping (NSDictionary) -> ()) {
    let request = URL(string: "\(url)?\(query(params as [String : AnyObject]))")!

    let session = URLSession.shared
    //let t = session.dataTask(with: <#T##URLRequest#>, completionHandler: <#T##(Data?, URLResponse?, Error?) -> Void#>)
    
    let task = session.dataTask(with: request, completionHandler: { (data: Data, response: HTTPURLResponse, error: Error) -> Void in
  ...
  fileprivate class func handleResponse(_ data: Data!, response: HTTPURLResponse!, error: Error!, success: @escaping (NSDictionary) -> ()) {
    ...
    //let serializationError: ErrorType!
